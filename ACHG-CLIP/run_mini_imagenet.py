...
from data.mini_imagenet_classes import get_mini_imagenet_class_names
...
def main():
    ...
    parser.add_argument("--short", action="store_true", help="Run short validation")
    parser.add_argument("--resume_base", type=str, default="", help="Path to checkpoint to resume base training from")
    parser.add_argument("--variant", type=str, default="", help="CLIP backbone variant")
    parser.add_argument("--seed", type=int, default=None, help="Seed for class split")
    ...
    parser.add_argument("--incremental_epochs", type=int, default=None)
    parser.add_argument("--base_batch_size", type=int, default=None)
    parser.add_argument("--incremental_batch_size", type=int, default=None)
    ...
    if not os.path.exists(data_root):
        for candidate in ["D:/FSCIL/datasets", "/kaggle/input", "./datasets"]:
    ...
    # ─── FIX: Get correct English class names for miniImageNet ───────────────
    # miniImageNet uses ImageFolder, so classes are WordNet IDs (e.g. "n01532829").
    # We must map these to English names before tokenizing for CLIP.
    raw_dataset = manager.train_dataset
    # Traverse through FSCILSubset wrapper to get the underlying ImageFolder
    if hasattr(raw_dataset, 'subset') and hasattr(raw_dataset.subset, 'dataset'):
        imagefolder = raw_dataset.subset.dataset
    elif hasattr(raw_dataset, 'dataset'):
        imagefolder = raw_dataset.dataset
    else:
        imagefolder = raw_dataset

    if hasattr(imagefolder, 'classes'):
        wordnet_ids = imagefolder.classes   # e.g. ['n01532829', 'n01558993', ...]
        class_names = get_mini_imagenet_class_names(wordnet_ids)
        print(f"Resolved {len(class_names)} class names from WordNet IDs.")
        print(f"  First 5: {class_names[:5]}")
    else:
        # Absolute last-resort fallback — generic names, still better than WordNet IDs
        print("WARNING: Could not extract class list from dataset. Using generic fallback names.")
        class_names = [f"object class {i}" for i in range(100)]
    # ─────────────────────────────────────────────────────────────────────────

    # Match HuggingFace model to the CLIP variant
    hf_model_map = {
    ...
    # Tokenize all 100 correct English class names
    text_prompts = [f"a photo of a {name}" for name in class_names]
    print(f"Sample text prompts: {text_prompts[:3]}")
    ...
    if variant in ["ViT-B/32", "ViT-B/16"]:
        d_model = 768; d_e = 512; d_k = 64; num_heads = 12; num_layers = 12; ffn_hidden_dim = 3072
    elif variant == "ViT-L/14":
        d_model = 1024; d_e = 768; d_k = 64; num_heads = 16; num_layers = 24; ffn_hidden_dim = 4096
    else:
        raise ValueError(f"Unknown variant: {variant}")

    clip_cfg = CLIPConfig(
        variant=variant, d_model=d_model, d_e=d_e, d_k=d_k,
        num_heads=num_heads, num_layers=num_layers, vocab_size=VOCAB_SIZE,
        max_text_len=MAX_TEXT_LEN, patch_dim=1, max_patches=1,
        ffn_hidden_dim=ffn_hidden_dim, dropout=0.0
    )
    achg_cfg = ACHGCLIPConfig(
        clip=clip_cfg, num_layers=num_layers, prompt_dim=d_model,
        node_feature_dim=d_model, mlp_bridge_hidden_dim=256, gin_num_layers=4,
        gin_hidden_dim=16, acga_latent_dim=64, hgnec_compressed_dim=8, hgnec_restored_dim=16,
    )
    ...
    evaluator = FSCILSessionEvaluator(model=eval_wrapper, device=device, data_manager=manager)
    ...
    seed_val = data_cfg.get('seed', {}).get('value', 42) if isinstance(data_cfg.get('seed'), dict) else data_cfg.get('seed', 42)
